...
import { HospitalRequest, HospitalRequestResponse } from "../entities/hospital-request";
import { HospitalRequestRepository } from "../interfaces/repositories/hospital-request-repository";

export class HospitalRequestRepositoryImpl implements HospitalRequestRepository {
    ...
    async create(hospital_request: HospitalRequest): Promise<boolean> {
        ...
        const result = await this.hospitalRequestDataSource.create(hospital_request, user_id)
        return result
    }
    ...
    updateResponseStatus(hospital_response: HospitalRequestResponse): Promise<boolean> {
